<template>
  <section :id="id" class="mb-10">
    ...
    <div class="max-w-screen-xl mx-auto">
      <p class="mb-9 text-center dark:text-dark-white">{{ (isEng ? 'Hope you enjoy my work!' : '希望您喜歡我的作品，若有興趣與我合作') }}<span class="block lg:inline">{{ (isEng ? 'if you’re interested in collaborating or have any suggestions, please feel free to contact me!' : '或有任何建議，都歡迎與我聯繫!') }}</span></p>
          ...
          <div class="w-full lg:w-3/4 p-5">
              <VForm action="https://VFormsubmit.co/89a36b0827909cfaa7405e0c39747ab3" v-slot="{ meta }" method="POST">
                <div class="grid mb-5 grid-cols-2 gap-4">
                  ...
                  <div>
                    <label for="Email" class="after:content-['*'] after:ml-0.5 after:text-red-500 block mb-2 text-sm font-bold dark:text-dark-white">信箱 / Email</label>
                ...
                <div class="mb-5">
                  <label for="Subject" class="after:content-['*'] after:ml-0.5 after:text-red-500 block mb-2 text-sm font-bold dark:text-dark-white">主題 / Subject</label>
                ...
                <div class="mb-5">
                  <label for="Message" class="after:content-['*'] after:ml-0.5 after:text-red-500 block mb-2 text-sm font-bold dark:text-dark-white">訊息 / Message</label>
                ...
                <div class="flex justify-center xl:justify-start mt-9">
                  <button type="submit" :disabled="!meta.valid" class="text-black bg-primary dark:bg-dark-primary disabled:text-black/70 dark:disabled:text-black/70 disabled:bg-primary/70 dark:disabled:bg-dark-primary/70 w-full xl:w-2/5 hover:bg-black hover:text-white focus:ring-4 focus:outline-none focus:ring-gray-300 font-bold rounded text-sm px-5 py-2.5 text-center shadow">{{ (isEng ? 'Send' : '寄出') }}</button>
                </div>
              </VForm>
          </div>
        </div>
